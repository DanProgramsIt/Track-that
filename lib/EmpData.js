...
class EmpData {
    ...
    getEmployeeById(id) {
        return query('SELECT * from employee WHERE employee.id = ?', [id]);
    }

    getEmployeesByDept(deptId) {
        return query('SELECT employee.id, employee.first_name, employee.last_name, role.title, role.salary, department.name AS department, CONCAT(manager.first_name, " ", manager.last_name) manager_name FROM ((employee LEFT JOIN employee manager ON manager.id = employee.manager_id) INNER JOIN role ON employee.role_id = role.id INNER JOIN department ON role.department_id = department.id) ORDER BY employee.id) WHERE department.id = ? ORDER BY employee.id', [deptId]);
    }

    getEmployeesByMgr(managerId) {
        return query('SELECT employee.id, employee.first_name, employee.last_name, role.title, role.salary, department.name AS department, CONCAT(manager.first_name, " ", manager.last_name) manager_name FROM ((employee LEFT JOIN employee manager ON manager.id = employee.manager_id) INNER JOIN role ON employee.role_id = role.id INNER JOIN department ON role.department_id = department.id) ORDER BY employee.id) WHERE employee.manager_id = ? ORDER BY employee.id', [managerId]);
    }

    getEmployeesByRole(roleId) {
        return query('SELECT employee.id, employee.first_name, employee.last_name, role.title, role.salary, department.name AS department, CONCAT(manager.first_name, " ", manager.last_name) manager_name FROM ((employee LEFT JOIN employee manager ON manager.id = employee.manager_id) INNER JOIN role ON employee.role_id = role.id INNER JOIN department ON role.department_id = department.id) ORDER BY employee.id) WHERE role.id = ? ORDER BY employee.id', [roleId]);
    }

    getEmployeesWithMgr(managerId) {
        return query('SELECT employee.id, employee.first_name, employee.manager_id FROM employee WHERE employee.manager_id = ? ORDER BY employee.id', [managerId]);
    }
    ...
    getDepartments() {
        return quary('SELECT department.id, department.name AS department FROM department');
    }
    ...
    getBudgetByDept(deptId) {
        return query('SELECT department.name AS department, SUM(sole.salary) AS Total_Department_Budget FROM (employee INNER JOIN role ON employee.role_id = role.id INNER JOIN department On role.department_id = department.id = ? GROUP BY department', [deptId])
    }
...
updateEmpRole(roleId, empId) {
    return query('UPDATE emplyee SET ? WHERE ?', [
        {
            role_id: roleId
        },
        {
